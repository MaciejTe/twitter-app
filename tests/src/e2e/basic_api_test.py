...
def test_insert_message(db_client):
    ...
    payload = {"text": "sample message", "tags": ["tag1", "tag2"]}
    ...
    count = db_client[settings.DB_COLLECTION_NAME].count_documents(
        {"_id": ObjectId(doc_id)}
    )
    assert count == 1
...
def test_get_messages_improper_query_param_from():
    ...
    query_params = {"from": "2912-12-54-a98wj"}
    response = requests.get(URL, params=query_params)
    assert response.status_code == 422
    assert response.json() == {
        "error": "failed to convert start filtering date to RFC3339 time format"
    }


def test_get_messages_improper_query_param_to():
    """Get messages using improper `from` query parameter. """
    query_params = {"to": "2912-12-54-a98wj"}
    response = requests.get(URL, params=query_params)
    assert response.status_code == 422
    assert response.json() == {
        "error": "failed to convert stop filtering date to RFC3339 time format"
    }
